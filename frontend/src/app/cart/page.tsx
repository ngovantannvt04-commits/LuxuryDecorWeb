...
export default function CartPage() {
  ...
  // Tự động chọn tất cả khi mới vào (hoặc giữ nguyên nếu muốn user tự chọn)
  // Ở đây mình để mặc định là chưa chọn gì hoặc chọn hết tùy bạn. 
  // Thường UX tốt là chọn hết:
  useEffect(() => {
    if (cartItems.length > 0 && selectedIds.length === 0) {
        // setSelectedIds(cartItems.map(item => item.productId)); // Uncomment nếu muốn mặc định chọn hết
    }
  }, [cartItems]);
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === cartItems.length) {
      ...
      setSelectedIds(cartItems.map((item) => item.productId)); // Chọn hết
    }
  };
  ...
  const handleCheckout = () => {
    ...
    // Chuyển sang trang checkout kèm theo danh sách ID các món cần mua
    const idsParam = selectedIds.join(",");
    router.push(`/checkout?items=${idsParam}`);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 h-fit bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <div className="flex justify-between text-gray-600">
                        <span>Tạm tính</span>
                        {/* Hiển thị giá của các món được chọn */}
                    ...
                    <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                        ...
                        <span className="font-bold text-amber-700 text-xl">{formatCurrency(selectedTotal)}</span>
                    </div>
                </div>

                {/* Sửa nút Link thành Button để xử lý logic check */}
                <button 
                    onClick={handleCheckout}
                    disabled={selectedIds.length === 0}
                    className="w-full bg-black text-white py-4 rounded-full font-bold hover:bg-gray-800 transition flex items-center justify-center gap-2 shadow-lg disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                    <CreditCard size={18} /> Mua hàng ({selectedIds.length})
                </button>
